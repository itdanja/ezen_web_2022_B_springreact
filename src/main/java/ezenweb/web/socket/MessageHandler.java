...
public class MessageHandler extends TextWebSocketHandler {
    ...
    private static List<WebSocketSession> 접속명단2 = new ArrayList<>();

    @Override   // 1. 클라이언트가 서버소켓으로 부터 접속했을때
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        log.info("afterConnectionEstablished : " + session );
        접속명단2.add( session ); // 클라이언트 세션이 들어왔을때 리스트에 저장 [ 왜???다른 세션과 통신하기 위해 ]
    }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        for( WebSocketSession 세션 : 접속명단2 ){ 세션.sendMessage( message );  }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        접속명단2.remove( session ); // 클라이언트 세션이 나갔을때 리스트에 제거
    }
}
